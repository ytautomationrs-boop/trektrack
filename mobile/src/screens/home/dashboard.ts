...
export function activeEvents<T extends {hasJoined:boolean;isHost:boolean;status:string}>(events:T[]){return events.filter(e=>(e.hasJoined||e.isHost)&&['UPCOMING','LIVE'].includes(e.status));}
