...
export function AstaLogo({size=96,width,height,backgroundColor='transparent',symbol=false,compact=false}:{size?:number;width?:number;height?:number;backgroundColor?:string;symbol?:boolean;compact?:boolean}){
 const w=width??size;
 if(compact)return <View accessible accessibilityLabel="ASTA logo" style={{width:w,height:height??44,flexDirection:'row',alignItems:'center',gap:6}}><Image source={require('../../assets/brand/asta-symbol-white.png')} style={{width:38,height:32}} resizeMode="contain"/><AstaLogo width={w-44}/></View>;
 const markHeight=w*0.2,scale=w/1060;
