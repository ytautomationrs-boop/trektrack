...
export function HealthDashboard({ raceId }: {raceId?:string}) {
 ...
 return <View style={s.card}>
   <Pressable accessibilityRole="button" accessibilityLabel="Your activity, show entered races" accessibilityState={{expanded}} onPress={()=>setExpanded(v=>!v)} style={{gap:10}}>
   <View style={s.row}><Ionicons name="heart" size={23} color={colors.fail}/><Text style={s.title}>Your activity</Text><Ionicons name={expanded?"chevron-up":"chevron-down"} size={22} color={colors.text}/></View>
   <View style={s.row}><View style={{flex:1}}><Text style={s.number}>{health.todaySteps==null?"—":health.todaySteps.toLocaleString()}</Text><Text style={s.body}>Steps today</Text></View><Ionicons name="footsteps" size={36} color={colors.text}/></View>
   <Text style={s.small}>{expanded?"Hide your entered races":"Tap to see your entered races"}</Text>
   </Pressable>
   {expanded && <>
