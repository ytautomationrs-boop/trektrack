...
| POST | `/races` | `{ name, metricKey, durationDays, format, visibility?, squadName?, squadJoinPolicy? }` | Creates a **private** race and enters the creator as racer 1, charging the league's fixed entry fee. `durationDays` is 1 or 7. **No fee, prize, field size or league is accepted from the request** — all four are platform config. `visibility: "PUBLIC"` returns `400 public_race_not_user_creatable`. |
