...
export async function poolRoutes(app:FastifyInstance){
  ...
  app.post<{Params:{id:string}}>('/pools/:id/test-advance',{preHandler:requireAuth},async req=>{const b=z.object({version:z.number().int().min(0)}).parse(req.body);return {pool:await advancePool(req.params.id,req.userId,b.version)};});
  let running=false;
