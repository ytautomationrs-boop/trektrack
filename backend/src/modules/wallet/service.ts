...
export async function requestWithdrawal(params: { userId: string; amountCents: number; destination: WithdrawalDestination }) {
  ...
  const { transferCode, status } = await initiateTransfer({ amountCents: params.amountCents, recipientCode, reason: "ASTA wallet withdrawal" });
      if (status === "otp") {
        throw new WalletError(
          "payout_needs_otp",
